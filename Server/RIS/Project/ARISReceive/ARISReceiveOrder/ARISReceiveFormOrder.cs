...
{
	public partial class ARISReceiveFormOrder : Form
	{
		#region field
		/// <summary>
		/// ログフォーム
		...
		private void ArisReceiveForm_Load(object sender, EventArgs e)
		{
			...
			portTextBox.Text = ConfigurationManager.AppSettings["Port"].StringToString();
			sendTimeoutTextBox.Text = ConfigurationManager.AppSettings["SendTimeout"].StringToString();
			receiveTimeoutTextBox.Text = ConfigurationManager.AppSettings["ReceiveTimeout"].StringToString();
			clientCloseTimeoutTextBox.Text = ConfigurationManager.AppSettings["ClientCloseTimeout"].StringToString();
			#endregion
		}
		...
		{
			return new TcpListenerHandlerOrder();
		}
	}
}
