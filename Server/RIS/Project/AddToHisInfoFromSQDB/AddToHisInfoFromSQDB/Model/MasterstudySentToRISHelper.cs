...
namespace AddToHisInfoFromSQDB.Model
{
	internal static class MasterstudySentToRISHelper
	{
		...
		/// <param name="cn"></param>
		/// <returns></returns>
		public static List<MasterStudy> GetDataFromDB(IDbConnection cn)
		{
		...
		public static void DeleteToDB(MasterStudy lastMasterStudy, IDbConnection cn, IDbTransaction tr)
		{
			using (IDbCommand command = cn.CreateCommand())
			{
                command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
		...
		public static void InsertToDB(List<MasterStudy> list, IDbConnection cn, IDbTransaction tr)
		{
			using (IDbCommand command = cn.CreateCommand())
			{
                command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
		...
		private static void InsertToDB(MasterStudy masterStudy, IDbCommand command)
		{
