using System;
using System.Linq;
using CoReports;
using StatisticsPrint.Data;
using StatisticsPrint.Model.COReports;
using StatisticsPrint.Model.Print.Bui.Data;
using StatisticsPrint.Model.Print.Common;
using StatisticsPrint.Model.Print.Common.Data;

namespace StatisticsPrint.Model.Print.Bui
{
	/// <summary>
	/// フォームファイル書き込みクラス
	/// </summary>
	public class BuiWriter : PrintWriterBase
	{
		#region 定数

		#region COReprtsオブジェクト
		
		/// <summary>
		/// アイテムリスト
		/// </summary>
		protected const string COREPORTS_EXAM_KIND_LIST = "ExamKindList";

		/// <summary>
		/// 部位名リスト
		/// </summary>
		protected const string COREPORTS_BUI_NAME = "BuiName";

		#endregion

		#endregion

		#region フィールド
		#endregion

		#region プロパティ

		/// <summary>
		/// データクラス
		/// </summary>
		public BuiItem Data
		{
			get;
			set;
		}

		#endregion

		#region メソッド

		#region public
		
		/// <summary>
		/// 書き込み
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		public override void Write(object sender, PrintEventArgs e)
		{
			CrForm f = e.crForm;
			//フィールドを全てクリアします
			f.ClearAllFields();

			WriteHeader(Condition, f);

			int rowIndex = 0;
			foreach (BuiItemExamRoom examRoomItem in Data.ChildItemList)
			{
				f.CrObjects[COREPORTS_EXAM_ROOM_NAME].Text = examRoomItem.KENSASITU_NAME;
				rowIndex = WriteBuiItemChild(f, rowIndex, examRoomItem);
			}
		}

		#endregion

		#region protected

		/// <summary>
		/// 部位Itemの子供書き込み
		/// </summary>
		/// <param name="f"></param>
		/// <param name="rowIndex"></param>
		/// <param name="buiItemChild"></param>
		/// <returns></returns>
		protected int WriteBuiItemChild(CrForm f, int rowIndex, IBuiItemChild buiItemChild)
		{
			foreach (BuiItemKensaType kensaTypeItem in buiItemChild.KensaTypeItemList)
			{
				foreach (BuiItemBui buiItem in kensaTypeItem.BuiItemList)
				{
					WriteDetail(f, rowIndex, kensaTypeItem, buiItem);

					f.CrObjects[COREPORTS_MONTH_TOTAL].get_CrListCell(0, rowIndex).Text = buiItem.Total; //小計(検査種別+部位分類)
					rowIndex = IncRow(rowIndex, f);
				}
				WriteSubtotalKensaTypeAndDay(kensaTypeItem, rowIndex, f); //小計(検査種別+日付)を書き込み

				rowIndex = NewPage(f); //部位は検査種別ごとに改ページ
			}
			return rowIndex;
		}

		/// <summary>
		/// ヘッダ書き込み
		/// </summary>
		/// <param name="condition"></param>
		/// <param name="crForm"></param>
		protected override void WriteHeader(ConditionPrint condition, CrForm crForm)
		{
			base.WriteHeader(condition, crForm);
			WriteHeaderPlace(condition, crForm);
		}

		/// <summary>
		/// 明細行クリア
		/// </summary>
		/// <param name="f"></param>
		protected override void ClearDetailFields(CrForm f)
		{
			CrObject crExamKindList = f.CrObjects[COREPORTS_EXAM_KIND_LIST];
			for (int rowIndex = 0; rowIndex <= crExamKindList.ListRows -1; rowIndex++)
			{
				f.CrObjects[COREPORTS_EXAM_KIND_LIST].get_CrListCell(0, rowIndex).Text = "";
				f.CrObjects[COREPORTS_BUI_NAME].get_CrListCell(0, rowIndex).Text = "";
				f.CrObjects[COREPORTS_MONTH_TOTAL].get_CrListCell(0, rowIndex).Text = "";

				DayLoopExecuter(delegate(DateTime currentDate)
				{

					SetDetailByDay(currentDate.Day, rowIndex, "", f);
				});
			}
		}
		#endregion

		#region private
		
		/// <summary>
		/// 明細行書き込み
		/// </summary>
		/// <param name="f"></param>
		/// <param name="rowIndex"></param>
		/// <param name="kensaTypeItem"></param>
		/// <param name="buiItem"></param>
		private void WriteDetail(CrForm f, int rowIndex, BuiItemKensaType kensaTypeItem,
			BuiItemBui buiItem)
		{
			f.CrObjects[COREPORTS_EXAM_KIND_LIST].get_CrListCell(0, rowIndex).Text = kensaTypeItem.KENSATYPE_NAME;
			f.CrObjects[COREPORTS_BUI_NAME].get_CrListCell(0, rowIndex).Text = buiItem.BUI_NAME;

			DayLoopExecuter(delegate(DateTime currentDate)
			{
				DetailDayItem dayitem = buiItem.DetailDayList.FirstOrDefault(
					d => d.EXAMENDDATE_YYYYMMDD10 == currentDate.ToString("yyyy/MM/dd"));
				if (dayitem == null)
				{
					return;
				}
				SetDetailByDay(currentDate.Day, rowIndex, dayitem.CNT, f);
			});
		}

		/// <summary>
		/// ヘッダ場所書き込み
		/// </summary>
		/// <param name="condtion"></param>
		/// <param name="crForm"></param>
		private void WriteHeaderPlace(ConditionPrint condtion, CrForm crForm)
		{
			ConditionPrintPlace conditionPlace = (ConditionPrintPlace)condtion;
			crForm.CrObjects[COREPORTS_PLACE_NAME].Text = conditionPlace.PlaceName;
		}

		/// <summary>
		/// 日付毎詳細設定
		/// </summary>
		/// <param name="day"></param>
		/// <param name="rowIndex"></param>
		/// <param name="value"></param>
		/// <param name="crForm"></param>
		private void SetDetailByDay(int day, int rowIndex, string value, CrForm crForm)
		{
			string objectName = string.Format(COREPORTS_DAY_DETAIL_NAME_FORMAT, day);
			crForm.CrObjects[objectName].get_CrListCell(0, rowIndex).Text = value;
		}

		/// <summary>
		/// 小計(検査種別+日付)小計を書き込み
		/// </summary>
		/// <param name="kensaTypeItem"></param>
		/// <param name="rowIndex"></param>
		/// <param name="f"></param>
		private void WriteSubtotalKensaTypeAndDay(BuiItemKensaType kensaTypeItem, int rowIndex, CrForm f)
		{
			f.CrObjects[COREPORTS_BUI_NAME].get_CrListCell(0, rowIndex).Text = "<日別合計>";

			DayLoopExecuter(delegate(DateTime currentDate)
			{
				DetailDayItem dayitem = kensaTypeItem.DetailDayList.FirstOrDefault(
					d => d.EXAMENDDATE_YYYYMMDD10 == currentDate.ToString("yyyy/MM/dd"));
				if (dayitem == null)
				{
					SetDetailByDay(currentDate.Day, rowIndex, "0", f);
					return;
				}
				SetDetailByDay(currentDate.Day, rowIndex, dayitem.CNT, f);
			});
			f.CrObjects[COREPORTS_MONTH_TOTAL].get_CrListCell(0, rowIndex).Text = kensaTypeItem.Total;
		}

		#endregion
		#endregion
	}
}
