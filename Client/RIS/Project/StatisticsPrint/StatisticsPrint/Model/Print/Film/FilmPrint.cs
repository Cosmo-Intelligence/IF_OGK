...
namespace StatisticsPrint.Model.Print.Film
{
	...
	/// フィルム一覧印刷
	/// </summary>
	internal class FilmPrint : PrintBase
	{
		#region プロパティ
		/// <summary>
		/// ドキュメント名
		...
		public override string FormFilePath
		{
			get
			{
				return ConfigurationManager.AppSettings["FormFileFilm"].StringToString();
			}
		}
		#endregion
		...
		public override void Print(ConditionPrint condition)
		{
			...
			FilmItem data = GetResultData(condition);
			List<FilmItemFilm> list = data.FilmItemList;
			if (list.Count < 1)
			{
			...
			{
				IPrintWriter writer = new FilmWriter()
				{
					Condition = condition,
					Data = data
				};
		...
		/// <param name="condition"></param>
		/// <returns></returns>
		private FilmItem GetResultData(ConditionPrint condition)
		{
		...
		/// <returns></returns>
		private FilmItem GetResultDataByCommand(ConditionPrintPlace conditionPlace, IDbCommand command)
		{
			command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
			command.CommandText = "STATISTICS_PRINT_PACKAGE.GetFilmData";
			...
			p = command.CreateParameter();
			((OracleParameter)p).SetOututRefCursor("P_FILM_CURSOR");
			command.Parameters.Add(p);
			#endregion
			...
			using (OracleRefCursor cur1 = (OracleRefCursor)cmd.Parameters["P_FILM_CURSOR"].Value)
			{
				using (OracleDataReader dr1 = cur1.GetDataReader())
				{
					FilmItem data = new FilmItem();
		...
		private void AddResultData(OracleDataReader dr1, FilmItem data)
		{
			...
			AddFilmList(dr1, data.FilmItemList);
		}

		private void AddFilmList(OracleDataReader dr1, List<FilmItemFilm> filmList)
		{
			string filmID = dr1.GetStringByDB("FILM_ID");
			FilmItemFilm filmItem = filmList.FirstOrDefault(b => b.FILM_ID == filmID);
			if (filmItem == null)
			{
				filmItem = new FilmItemFilm();
				filmItem.FILM_ID = dr1.GetStringByDB("FILM_ID");
				filmItem.FILM_NAME = dr1.GetStringByDB("FILM_NAME");
				filmItem.FILM_SHOWORDER = dr1.GetStringByDB("FILM_SHOWORDER");
				filmList.Add(filmItem);
			}

			string gyoSyosai = dr1.GetStringByDB("GYO_SYOSAI");
			const string GYO_SYOSAI_TOTAL_KUBUN = "小計(フィルム)";
			if (GYO_SYOSAI_TOTAL_KUBUN == gyoSyosai)
			{
				filmItem.Total = dr1.GetStringByDB("CNT");
				return;
			}

			filmItem.AddDay(dr1.GetStringByDB("EXAMENDDATE_YYYYMMDD10"), dr1.GetStringByDB("CNT"));
		}
