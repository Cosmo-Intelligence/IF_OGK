...
namespace StatisticsPrint.Model.Print.CDFilm
{
	...
	/// Film読込・書出/CD読込・書出印刷
	/// </summary>
	internal class CDFilmPrint : PrintBase
	{
		#region プロパティ
		/// <summary>
		/// ドキュメント名
		...
		public override string FormFilePath
		{
			get
			{
				return ConfigurationManager.AppSettings["FormFileCDFilm"].StringToString();
			}
		}
		#endregion
		...
		public override void Print(ConditionPrint condition)
		{
			...
			CDFilmItem data = GetResultData(condition);
			List<CDFilmItemCDFilm> list = data.CDFilmItemList;
			if (list.Count < 1)
			{
			...
			{
				IPrintWriter writer = new CDFilmWriter()
				{
					Condition = condition,
					Data = data
				};
		...
		/// <param name="condition"></param>
		/// <returns></returns>
		private CDFilmItem GetResultData(ConditionPrint condition)
		{
		...
		/// <returns></returns>
		private CDFilmItem GetResultDataByCommand(ConditionPrint condition, IDbCommand command)
		{
			command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
			command.CommandText = "STATISTICS_PRINT_PACKAGE.GetCDFilmData";
			...
			((OracleParameter)p).SetOututRefCursor("P_CD_FILM_CURSOR");
			command.Parameters.Add(p);
			#endregion
			...
			using (OracleRefCursor cur1 = (OracleRefCursor)cmd.Parameters["P_CD_FILM_CURSOR"].Value)
			{
				using (OracleDataReader dr1 = cur1.GetDataReader())
				{
					CDFilmItem data = new CDFilmItem();
		...
		private void AddResultData(OracleDataReader dr1, CDFilmItem data)
		{
			...
			AddCDFilmList(dr1, data.CDFilmItemList);
		}

		private void AddCDFilmList(OracleDataReader dr1, List<CDFilmItemCDFilm> emergencyList)
		{
			string kbnName = dr1.GetStringByDB("KBN_NAME");
			CDFilmItemCDFilm emergencyItem = emergencyList.FirstOrDefault(b => b.KBN_NAME == kbnName);
			if (emergencyItem == null)
			{
				emergencyItem = new CDFilmItemCDFilm();
