...
namespace StatisticsPrint.Model.Print.RI
{
	...
	/// RI　分類別集計表印刷
	/// </summary>
	internal class RIPrint : PrintBase
	{
		#region プロパティ
		/// <summary>
		/// ドキュメント名
		...
		public override string FormFilePath
		{
			get
			{
				return ConfigurationManager.AppSettings["FormFileRI"].StringToString();
			}
		}
		#endregion
		...
		public override void Print(ConditionPrint condition)
		{
			...
			RIItem data = GetResultData(condition);
			List<RIItemRI> list = data.RIItemList;
			if (list.Count < 1)
			{
			...
			{
				IPrintWriter writer = new RIWriter()
				{
					Condition = condition,
					Data = data
				};
		...
		/// <param name="condition"></param>
		/// <returns></returns>
		private RIItem GetResultData(ConditionPrint condition)
		{
		...
		/// <returns></returns>
		private RIItem GetResultDataByCommand(ConditionPrint condition, IDbCommand command)
		{
			command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
			command.CommandText = "STATISTICS_PRINT_PACKAGE.GetRIData";
			...
			((OracleParameter)p).SetOututRefCursor("P_RI_CURSOR");
			command.Parameters.Add(p);
			#endregion
			...
			using (OracleRefCursor cur1 = (OracleRefCursor)cmd.Parameters["P_RI_CURSOR"].Value)
			{
				using (OracleDataReader dr1 = cur1.GetDataReader())
				{
					RIItem data = new RIItem();
		...
		private void AddResultData(OracleDataReader dr1, RIItem data)
		{
			...
			AddRIList(dr1, data.RIItemList);
		}

		private void AddRIList(OracleDataReader dr1, List<RIItemRI> kbnList)
		{
			string kbnName = dr1.GetStringByDB("KBN_NAME");
			RIItemRI kbnItem = kbnList.FirstOrDefault(b => b.KBN_NAME == kbnName);
			if (kbnItem == null)
			{
				kbnItem = new RIItemRI();
