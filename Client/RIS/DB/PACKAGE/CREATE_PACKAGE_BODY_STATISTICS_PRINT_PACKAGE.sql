CREATE OR REPLACE PACKAGE BODY STATISTICS_PRINT_PACKAGE AS
	...
	END GetBuiBunruiData;

	--ŽB‰eŒŸ¸”ˆê——iŽåsˆ×j
	--ŽÀŽ{êŠ‘I‘ðŽž
	--C³Žž‚ÍGetBuiDataAll‚àŒ©’¼‚·‚±‚Æ
	...
	PROCEDURE GetGeppoData(
		...
		v_target_month CHAR(6);
	BEGIN
		OPEN P_GEPPO_CURSOR FOR 
		SELECT
			CASE t.gid
				WHEN 0 THEN GYO_DETAIL
		...
		--‘O”N‚Ìyyyymm•¶Žš—ñ‚É•ÏŠ·
		SELECT TO_CHAR(ADD_MONTHS(P_EXAMENDDATE, -12), 'YYYYMM') INTO v_target_month FROM DUAL;

		OPEN P_GEPPO_PREV_CURSOR FOR 
		SELECT
		...
		WHERE
			s.target_month = v_target_month; --yyyyMM‚Ì•¶Žš—ñ
